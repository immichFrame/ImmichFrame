        ...
        IList<IImmichFrameLogic> accounts)
    {
        ...
        var taskList = assetLists
            ...
            .Select(async tuple =>
            {
                ...
                return (account, (await task).TakeProportional(proportion));
            });
...
{
    public static IEnumerable<T> TakeProportional<T>(this IEnumerable<T> list, double proportion)
    {
        if (list == null) return Enumerable.Empty<T>();
        if (proportion <= 0) return Enumerable.Empty<T>();

        int totalCount = list.Count();
        int itemsToTake =
            (int)Math.Round(totalCount * proportion); // Or Math.Floor, Math.Ceiling, depending on desired rounding
